...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    static final int spread(int h) {
        return (h ^ (h >>> 16)) & HASH_BITS;  // HASH_BITS 确保hash值位正
    }
    ...
    public V get(Object key) {
        ...
        int h = spread(key.hashCode()); // 得到的一个正的hash值
        if ((tab = table) != null && (n = tab.length) > 0 &&
                (e = tabAt(tab, (n - 1) & h)) != null) { // 能够找到key对应的数组下标，并且数据不为空
            if ((eh = e.hash) == h) { // hash数组下标对应的头节点作比较
                if ((ek = e.key) == key || (ek != null && key.equals(ek))) // 如果头结点相同，直接返回对应的value
                    return e.val;
            }
            else if (eh < 0) // 如果是红黑树，以红黑树或者ForwardingNode(扩容时)的方式查找
                return (p = e.find(h, key)) != null ? p.val : null;
            while ((e = e.next) != null) { // 否则。以链表的方式寻找
                if (e.hash == h &&
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)
                tab = initTable();
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {  // 如果当前的hash地址没有数据，则支架在该节点插入数据
                if (casTabAt(tab, i, null,        // 通过cas控制，保证只有一个线程能插入成功
                        new Node<K,V>(hash, key, value, null)))
                    break;                   // no lock when adding to empty bin
            }
            else if ((fh = f.hash) == MOVED)
                tab = helpTransfer(tab, f);  // 如果检测到正在扩容，则帮助扩容
            else { // 此时，正常插入数据
                V oldVal = null;
                synchronized (f) {  // 锁住在链表或树中插入数据的代码
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) {  // 节点下面是链表
                            binCount = 1;
                            for (Node<K,V> e = f;; ++binCount) {  // 遍历该链表
                                ...
                                if (e.hash == hash &&
                                        ((ek = e.key) == key ||
                                                (ek != null && key.equals(ek)))) {  // 找到有没有相同的key,
                                    oldVal = e.val;    // 如果有，就更新value
                                    if (!onlyIfAbsent)
                                        e.val = value;
                                    break;
                                }
                                Node<K,V> pred = e;
                                if ((e = e.next) == null) { // 如果到了链表末尾还没找到相同的key,就把数据插入到链表的末尾
                            ...
                            Node<K,V> p;
                            binCount = 2;
                            if ((p = ((TreeBin<K,V>)f).putTreeVal(hash, key,  // 以树的方式插入数据
                                    value)) != null) {
                                oldVal = p.val;
                                if (!onlyIfAbsent)
                                    p.val = value;
                ...
                if (binCount != 0) {// 此时插入的如果是链表，要判断下是否要转化位红黑树
     ...
     * {@link java.util.concurrent.ConcurrentHashMap.KeySetView}.
     *
    ...
    @dalvik.annotation.codegen.CovariantReturnType(returnType = java.util.concurrent.ConcurrentHashMap.KeySetView.class, presentAfter = 28)
    public Set<K> keySet() {
